...
class TextCNNRNN(object):
    ...
    def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
        ...
        inputs = []
        for input_ in tf.split(pooled_concat, num_or_size_splits=int(reduced), axis=1):
            inputs.append(tf.squeeze(input_, [1]))
        ...
        # Collect the appropriate last words into variable output
        # (dimension = batch x embedding_size)
        output = outputs[0]
        ...
        with tf.name_scope('output'):
            self.W = tf.Variable(tf.truncated_normal([hidden_unit, num_classes], stddev=0.1),
                                 name='W')
